...
from location.forms import MapForm, SearchMapForm, LocationForm, RegistrationForm
...
def create_info_content(location):
    ...
    activities += '</ul>'    
    return '<div style="height:130px; width:300px;">%s</br>%s %s</br>%s, %s%s</div>' % (location.place_name, location.user.first_name, 
        location.user.last_name, location.phone_number, location.user.email, activities)
